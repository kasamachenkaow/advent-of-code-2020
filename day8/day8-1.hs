...
getSequentInstructionsFrom :: [(String, Int)] -> Int -> [Int] -> [(String, Int)]
getSequentInstructionsFrom instructions offset history =
  if (offset `notElem` history) && (offset < length instructions)
    then instruction : getSequentInstructionsFrom instructions newOffset (offset : history)
    else []
  where
...
main = do
  ...
  let nextInstructions = getSequentInstructionsFrom instructions 0 []
