...
validateValue ("byr", value) = matchTest regex value where
  regex = makeRegex "^(19[2-9][0-9]|200[0-2])$" :: Regex
validateValue ("cid", value) = True
validateValue ("ecl", value) = matchTest regex value where
  regex = makeRegex "^(amb|blu|brn|gry|grn|hzl|oth)$" :: Regex
validateValue ("eyr", value) = matchTest regex value where
  regex = makeRegex "^20([2][0-9]|30)$" :: Regex
validateValue ("hcl", value) = matchTest regex value where
  regex = makeRegex "^#[0-9a-f]{6}$" :: Regex
validateValue ("hgt", value) = matchTest regex value where
  regex = makeRegex "^(1[5-8][0-9]cm|19[0-3]cm|5[89]in|6[0-9]in|7[0-6]in)$" :: Regex
validateValue ("iyr", value) = matchTest regex value where
  regex = makeRegex "^20([1][0-9]|20)$" :: Regex
validateValue ("pid", value) = matchTest regex value where
  regex = makeRegex "^[0-9]{9}$" :: Regex
validateValue (xs, value) = False
